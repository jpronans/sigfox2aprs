<?php
  ...
  // Device ID
  $_id = $_GET["id"];
  // Latitute (Integer, no good for accurate position)
  $_lat = $_GET["lat"];
  // Longitude (Integer)
  $_lng = $_GET["lng"];
  // Sigfox data packet
  ...
  // UNIX Time stamp
  $_time = $_GET["time"];
  // Message received from more than one base station
  $_duplicate = $_GET["duplicate"];
  // Signal to Noise Ratio in db 
  $_snr = $_GET["snr"];
  // Base station identifier in HEX
  $_station = $_GET["station"];
  // RSSI in dBm
  $_rssi = $_GET["rssi"];
  // Average SNR from last 25 messages
  $_avgSnr = $_GET["avgSnr"];
  // Sequence Number of Message if Available.
    ...
    if(substr($_GET["data"],7,12) != "ffffffffffff")
      {
        ...
        // Write it out
        $_filedata = "ID: ".$_id.", Preamble: ".$_preamble.", Lat: " .$lat_aprs. ", Long: ".$lng_aprs. ", Sats: ".$sats.", HDOP: ".$hdop."\r\n";
        ...
        if ($mqtt->connect()) {
          ...
          $mqtt->publish("sigfox/survey",$_time.":".$_id.":".$_station.":".$_duplicate.":".$_rssi.":".$_avgSnr.":".$_seqNumber,0);
	        $mqtt->close();
        }
      }
      else
